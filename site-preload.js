      ...
      while (queue.length && valid()) {
        ...
        if (/\.html(?:$|\?)/i.test(url)) {
          ...
          if (!new URL(url).search) {
            const embedded = new URL(url); embedded.searchParams.set('embedded', '1'); embedded.searchParams.set('nav-version', window.siteNavigationVersion || '20260923-28'); enqueue(embedded.href);
          }
        } else {

...
- **视频按需加载**：先按界面语言选择默认平台，再以地区检测结果调整；访客可手动选择 YouTube 或哔哩哔哩。点击封面后才加载播放器，手动选择优先于自动检测，并始终保留原站链接和重新加载入口。切换视频、平台或进入项目页会停止旧播放器。
...
站点用 `localStorage` 保存语言偏好、手选视频平台和访问提示展示时间，用 `sessionStorage` 保存背景音乐会话状态；地区识别结果不会持久化。地区检测依次使用 `api.country.is`、`ipapi.co`，无共享检测任务时访问提示使用 `ipwho.is`。没有手选平台时，识别为中国大陆会选择哔哩哔哩，其余已识别地区选择 YouTube；检测失败保留按界面语言选择的默认平台。内嵌项目页复用父页检测结果，不重复查询或弹出提示。
...
- 手机菜单、语言菜单键盘操作、二维码放大／关闭／复制可用；视频首屏不创建 iframe，手选平台、重载和外链可用。
- 背景音乐跨项目连续，加载视频会静音；进入详情后首页视频停止，背景粒子暂停；浏览器往返缓存和减少动态效果偏好可恢复。
...
├── regional-video.js / video-player.css # 视频平台选择、按需加载与失败回退

...
```sh
node --test tests/particle-input.test.cjs tests/project-audio.test.cjs tests/nav-glass-optics.test.cjs tests/nav-glass-motion.test.cjs
git diff --check
```
...
[玻璃光学回归测试](tests/nav-glass-optics.test.cjs) 检查胶囊导航与圆角矩形卡片在八种尺寸下的中心中性、镜像对称、平直边不产生横向拉扯、调整宽度不拉伸光学场，以及 RGB 三通道在连续计算和 8 位位移图双线性采样后均不翻折。它验证光学映射约束，浏览器的合成与 Safari 页面捕获仍需实测。[滚动状态测试](tests/nav-glass-motion.test.cjs) 检查触摸前隐藏、惯性与反向滚动、多点触摸、视口变化，以及延迟回调清理。
...
├── nav-glass-motion.js          # 快照玻璃的触摸、惯性滚动和稳定帧保护
...
- **Safari 滚动防重影**：触摸开始、滚轮、页面滚动与可视视口变化时，立即隐藏 WebGL 快照层，以浏览器实时背景保留透明度与边缘高光；手指离开、连续 180 ms 无滚动且完成两帧绘制后，恢复已就绪的 RGB 折射。惯性滚动和反向滑动会延长等待，不叠加旧帧、不交叉淡化背景。这个阶段没有 RGB 位移折射；Chromium 原生 SVG 背景滤镜不受此切换影响。表面白色覆盖与高光只绘制一次，避免切换时重复增亮。

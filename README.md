...
- **顶部布局**：首页与二级页沿用同一液态玻璃导航。SVG 位移滤镜只处理圆角栏的边缘，中央模糊层保持文字清晰；不支持该滤镜时使用普通毛玻璃。二级页左侧为项目品牌或返回入口，中间为目录，右侧为音乐与语言控制；桌面目录居中，窄屏收进顶部菜单，正文不重复设置目录按钮。
...
├── nav-liquid-glass.css / .js    # 五页顶部玻璃折射、边缘色散与悬停高光
...
- **顶部导航、卡片与三语**：五页顶部导航参考 [yimingxi.art](https://www.yimingxi.art/) 的公开前端实现，采用 SVG RGB 位移图产生圆角边缘的液态玻璃折射；根据本站长导航栏缩小位移强度，中央保持清晰毛玻璃。悬停高光与当前章节的蓝色 Section Indicator 分层显示，窄屏菜单保留原有交互。文本卡片采用首页的静态圆角玻璃表面，统一边框、阴影与间距，无鼠标跟随光泽或卡片悬浮抬升。主站支持简体中文、繁體中文（香港用语）和 English。
...
| 液态玻璃导航 | [nav-liquid-glass.css](nav-liquid-glass.css)、[nav-liquid-glass.js](nav-liquid-glass.js)；检查五页的边缘折射、回退模糊、悬停高光、当前章节标记、移动菜单及内嵌详情。 |
